# original author:   H. Hergert
# original version:  1.1.0
# original date:     Nov 18, 2016
# tested with Python v2.7
# Solves the pairing model for four particles in a basis of four doubly degenerate states by means of a Similarity
# Renormalization Group (SRG) flow.
#
# This version contains some small changes to the original version so that the output figure could be the same as in the textbook
# Ruxin Zhang, May 2019
# ------------------------------------------------------------------------------
...
def main():
    ...
    ys = euler(derivative, y0, flowparams, 0.001, args=(dim,))
...
def plot_diagonals(data, eigenvalues, flowparams, delta, g):
    ...
    plt.savefig("srg_pairing_diag_delta%2.1f_g%2.1f_euler.pdf" % (delta, g), bbox_inches="tight", pad_inches=0.05)
    plt.show()
    ...
    plt.savefig("srg_pairing_diag-eval_delta%2.1f_g%2.1f_euler.pdf" % (delta, g), bbox_inches="tight", pad_inches=0.050)
    plt.show()
    return
